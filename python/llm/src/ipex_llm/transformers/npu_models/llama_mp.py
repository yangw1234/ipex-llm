...
from ipex_llm.transformers.npu_models.mp_model_base import LowBitBaseMultiDecoderGraph
from ipex_llm.transformers.npu_models.mp_model_base import BaseMultiDecoderlayerDecode
from ipex_llm.transformers.npu_models.mp_model_base import BaseSingleDecoderlayerPrefill


class LlamaMultiDecoderGraph(LowBitBaseMultiDecoderGraph):
    pass


class LlamaMultiDecoderlayerDecode(BaseMultiDecoderlayerDecode):
    ...
    def __init__(
        ...
        super().__init__(
            npu_backend_graph_cls=LlamaMultiDecoderGraph,
            parameters=parameters,
            input_laynorm_weights=input_laynorm_weights,
    ...
    @classmethod
    def from_torch(cls, torch_layers, max_output_len, intra_stages, transpose_value_cache):
        num_heads = torch_layers[0].self_attn.num_heads
        num_key_value_heads = torch_layers[0].self_attn.num_key_value_heads
        head_dim = torch_layers[0].self_attn.head_dim
        rms_norm_eps = torch_layers[0].input_layernorm.variance_epsilon
        intermediate_size = torch_layers[0].mlp.intermediate_size
        deocderlayers = []
        layer_weights = []
        input_layer_norm_weights = []
        post_attn_layernorm_weights = []
        layer_indexes = []
        for curr_layer in torch_layers:
            ...
            layer_indexes.append(curr_layer.self_attn.layer_idx)
        return cls(
...
class LlamaSingleDecoderlayerPrefill(BaseSingleDecoderlayerPrefill):
    ...
    @classmethod
    def from_torch(cls, torch_layer, max_output_len, transpose_value_cache):
        attn_layer = torch_layer.self_attn
        mlp_layer = torch_layer.mlp
        ...
        cached_cos = torch_layer.self_attn.rotary_emb.cos_cached.to(torch.float16)
        cached_sin = torch_layer.self_attn.rotary_emb.sin_cached.to(torch.float16)

        layer_norm_0 = torch_layer.input_layernorm.weight.to(torch.float16)
        layer_norm_1 = torch_layer.post_attention_layernorm.weight.to(torch.float16)

        num_heads = attn_layer.num_heads
        num_key_value_heads = attn_layer.num_key_value_heads
        ...
        return cls(
            ...
            layer_norm_0=layer_norm_0,
            layer_norm_1=layer_norm_1,
            layer_idx=attn_layer.layer_idx,
            rms_norm_eps=torch_layer.input_layernorm.variance_epsilon,
            intermediate_size=torch_layer.mlp.intermediate_size,
            max_seq_len=max_output_len,
            transpose_value=transpose_value_cache,
        )


class LlamaMultiDecoderLayerPrefill(torch.nn.Module):
    ...
    @classmethod
    def from_torch(cls, torch_layers, max_output_len, transpose_value_cache):
        deocderlayers = []
        for curr_layer in torch_layers:
            new_decoderlayer = LlamaSingleDecoderlayerPrefill.from_torch(
                curr_layer, max_output_len, transpose_value_cache
            )
            deocderlayers.append(new_decoderlayer)
        return cls(deocderlayers, max_output_len, transpose_value_cache)
...
def run_decode(
    ...
    multi_decoder = LlamaMultiDecoderlayerDecode.from_torch(
        model.model.layers[layer_start:layer_end], max_seq_len, intra_stages, transpose_value_cache
    )
    ...
    hidden_states = torch.empty(
        (1, 1, multi_decoder.head_dim * multi_decoder.num_heads), dtype=torch.float16
    )
...
def run_prefill(
    ...
    prefill_layers = LlamaMultiDecoderLayerPrefill.from_torch(
        model.model.layers, max_output_len, transpose_value_cache
    )
    ...
    while True:
        ...
        with torch.inference_mode():
            hidden_states, next_decoder_cache = prefill_layers(
                hidden_states,
